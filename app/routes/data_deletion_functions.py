...
def delete_city():
    ...
    if form.validate_on_submit(): 
        ...
        choise_field = form.city.input_field.data
        model = City
        if choise_field:
            ...
            return redirect(url_for('confirmation_form', table_name=table_name, record_id=record_id, redirect_to_function=redirect_to_function ))
...
@app.route('/confirmation_form/<table_name>/<int:record_id>/<redirect_to_function>', methods=['GET', 'POST'])
def confirmation_form(table_name, record_id, redirect_to_function):
    form = ConfirmationForm()
    flash(f'in confirmation_form! table_name: {table_name}, record_id: {record_id},  redirect_to_function: {redirect_to_function}')
